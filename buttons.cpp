...
Style::drawRadioOrCheckBox(const QStyleOption *option, QPainter *painter, const QWidget *widget, bool isRadio) const
{
    ...
    bool isOn = (option->state & State_On) || (!isRadio && !(option->state & State_Off));
    if (isOn)
        hover = false;
...
//         painter->setPen(QPen(FX::blend(FCOLOR(Window), FCOLOR(WindowText)), F(2) & ~1, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
    ...
    if (isOn) { // the drop
        ...
        if (hasFocus) {
            ...
            c.setAlpha((MAX_STEPS - animStep)*255/MAX_STEPS);
            painter->setPen(c);
            painter->setBrush(FX::blend(FCOLOR(WindowText), FCOLOR(HighlightedText), MAX_STEPS - animStep, animStep));
        } else {
            painter->setPen(Qt::NoPen);
