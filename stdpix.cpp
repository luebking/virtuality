...
#endif
paint:
        const QColor c = FX::blend(pal.color(fg), pal.color(bg), (sz > 16) ? 16 : 2, sunken ? 2 : (hover ? 4 : 2) );
...
//         break;
    ...
    case SP_FileDialogNewFolder: //  31
    {
        ...
        painter.setPen(QPen(FX::blend(pal.color(bg), pal.color(fg)), t/2.0));
...
//    case SP_FileDialogEnd: //  29
    ...
    {
        const float t = rect.width()/8.0;
        painter.setPen(QPen(FX::blend(pal.color(bg), pal.color(fg)), t));
        painter.setBrush(Qt::NoBrush);
        painter.setRenderHint ( QPainter::Antialiasing );
        float y = t;
...
//    case SP_FileDialogContentsView: //  34
    case SP_FileDialogListView: //  35
    {
        ...
        painter.setPen(Qt::NoPen);
        painter.setBrush(FX::blend(pal.color(bg), pal.color(fg)));
        painter.setRenderHint(QPainter::Antialiasing);
...
//         break;
    ...
    case SP_ToolBarVerticalExtensionButton: //  27  Extension button for vertical toolbars
        painter.setPen(Qt::NoPen);
        painter.setBrush(FX::blend(COLOR(Window), COLOR(WindowText)));
